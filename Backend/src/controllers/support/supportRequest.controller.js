...
if (lastRequest?.ticketNumber) {
  const match = lastRequest.ticketNumber.match(/\d+$/); // extract number safely
...
const assignTicket = async (req, res) => {
  try {
    ...
    // ← fetch logged-in user from DB to get name + email reliably
    ...
    console.log("loggedInUser =>", loggedInUser); // ← check what fields exist
