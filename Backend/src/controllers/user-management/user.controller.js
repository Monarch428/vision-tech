  ...
  try {
    ...
    const userWithSubscription = await Promise.all(
      users.map(async (user) => {
        // ✅ Removed the dead semicolon block
        ...
        return {
          ...
          email: user.email,
          role: user.role,
          subscription: user.deletedAt ? 'N/A' : (subscription?.plan?.name || 'Free'),
          status: user.isActive ? 'active' : 'deactivated',
          isActive:user.isActive,
          lastLogin: user.lastLogin ?? null,                    // ✅ directly from lean user
          lastLoginText: formatLastLogin(user.lastLogin),       // ✅ will now format correctly
