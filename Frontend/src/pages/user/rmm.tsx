...
// const CopyIcon = ({ color = "#374151", size = 14 }: { color?: string; size?: number }) => (
//   <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8">
...
//       setResult(res.agentConfig); // { deviceId, token } — shown once, for the agent's .env
//     } catch (err: any) {
...
//   const copyToClipboard = (text: string, which: "id" | "token") => {
...
//             {result ? "Device Added" : "Add New Device"}
//           </h2>
...
//               Copy these into the <code className="bg-gray-100 px-1 rounded">.env</code> file of the
//               monitoring agent installed on this device. The token is shown only once.
//             </p>

//             <div className="flex flex-col gap-3">
//               <div>
//                 <label className="text-xs font-medium text-gray-500 block mb-1">DEVICE_ID</label>
//                 <div className="flex items-center gap-2 bg-gray-50 border border-gray-300 rounded-lg px-3 py-2">
//                   <code className="text-xs text-gray-800 truncate flex-1">{result.deviceId}</code>
//                   <button onClick={() => copyToClipboard(result.deviceId, "id")} className="shrink-0">
//                     <CopyIcon />
//                   </button>
//                 </div>
//                 {copied === "id" && <p className="text-xs text-green-600 mt-1">Copied!</p>}
//               </div>

//               <div>
//                 <label className="text-xs font-medium text-gray-500 block mb-1">DEVICE_TOKEN</label>
//                 <div className="flex items-center gap-2 bg-gray-50 border border-gray-300 rounded-lg px-3 py-2">
//                   <code className="text-xs text-gray-800 truncate flex-1">{result.token}</code>
//                   <button onClick={() => copyToClipboard(result.token, "token")} className="shrink-0">
//                     <CopyIcon />
//                   </button>
//                 </div>
//                 {copied === "token" && <p className="text-xs text-green-600 mt-1">Copied!</p>}
//               </div>
//             </div>

//             <button
//               onClick={onClose}
//               className="w-full mt-5 border border-gray-300 text-gray-800 font-semibold text-sm py-2.5 rounded-xl hover:bg-gray-50 transition-colors"
//             >
//               Done
//             </button>
//           </>
