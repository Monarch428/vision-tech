          ...
          <div className="px-5 py-4 space-y-3">
            ...
            >
              <div className="flex items-start gap-2.5">
                <div
                  className={`w-[17px] h-[17px] rounded-full border-2 mt-0.5 flex-shrink-0 flex items-center justify-center transition-colors ${
                    selected === "now" ? "border-green-500" : "border-gray-300"
                  }`}
                >
                  {selected === "now" && (
...
function PayPalModal({
  ...
  const [step, setStep] = useState<"login" | "review" | "processing" | "done">(
    "login",
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[400px] mx-auto overflow-hidden">
        {/* PayPal header */}
        <div className="bg-[#003087] px-5 pt-5 pb-5">
          <div className="flex items-center justify-between mb-3">
            <svg viewBox="0 0 100 32" width="76" fill="white">
              <text
                x="0"
                y="24"
                fontSize="26"
          ...
          <p className="text-white/80 text-xs mb-2.5">
            Pay securely with your PayPal account
          </p>
          <div className="bg-white/10 rounded-xl px-3.5 py-2.5 flex items-center justify-between">
            <span className="text-white/80 text-sm">{plan.label} Plan</span>
          ...
          <p className="text-white/50 text-[11px] mt-2">
            {activateNow
              ? "Activates immediately · current plan cancelled"
              : "Activates after your current plan ends"}
          </p>
        </div>
          ...
          {step === "login" && (
            <div className="space-y-3">
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#003087]/30 focus:border-[#003087]"
              />
              <input
                type="password"
              ...
              <button
                onClick={handleLogin}
                className="w-full bg-[#0070ba] hover:bg-[#005ea6] text-white font-semibold rounded-xl py-2.5 text-sm transition-colors"
              >
                Log In
              </button>
              ...
              <div className="bg-gray-50 rounded-xl p-3.5 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {activateNow ? "Today" : "After current plan ends"}
                  </span>
                </div>
                <div className="border-t border-gray-200 pt-2 flex justify-between font-semibold">
                  <span>Total today</span>
                  <span>{activateNow ? plan.display : "$0.00"}</span>
                </div>
              </div>
              <div className="flex items-center gap-2 text-xs text-gray-400 bg-gray-50 rounded-xl px-3.5 py-2.5">
                <svg
                  width="13"
                  height="13"
                  fill="none"
                ...
                <span>256-bit SSL encryption · Demo transaction</span>
              </div>
              <button
                onClick={handlePay}
                className="w-full bg-[#ffc439] hover:bg-[#f0b429] text-[#003087] font-bold rounded-xl py-2.5 text-sm transition-colors"
              >
                {activateNow ? "Pay Now with PayPal" : "Confirm with PayPal"}
              </button>
            </div>
          )}

          {step === "processing" && (
            <div className="flex flex-col items-center justify-center py-7 space-y-3">
              <div className="w-11 h-11 rounded-full bg-blue-50 flex items-center justify-center">
                <SpinnerIcon />
              </div>
              <p className="font-medium text-gray-700 text-sm">Processing...</p>
            ...
            <div className="flex flex-col items-center justify-center py-7 space-y-3">
              ...
              <div className="text-center">
                <p className="font-bold text-gray-800">
                  {activateNow ? "Payment Successful!" : "Plan Scheduled!"}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {activateNow
                    ? `You're now on the ${plan.label} plan`
                    : `${plan.label} plan activates after your current plan ends`}
                </p>
              </div>
              <button
                onClick={onClose}
                className="w-full bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl py-2.5 text-sm transition-colors"
              >
              ...
              {!isCancelled ? (
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    disabled={cancelling}
                    className="flex items-center gap-2 px-4 py-2 border border-red-200 text-red-500 text-sm font-semibold rounded-xl hover:bg-red-50 transition-colors disabled:opacity-50"
                  >
          ...
          {invoicesLoading ? (
            ...
            <div className="space-y-3">
              {invoices.map((inv, index) => (
                <div
                  key={inv._id}
                  className="flex items-center gap-3 p-3.5 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors"
                >
