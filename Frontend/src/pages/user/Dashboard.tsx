...
// Unlimited support badge — shown instead of UsageBar for enterprise
function UnlimitedBadge({ usedMinutes }: { usedMinutes: number }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-violet-50 border border-violet-100 rounded-xl">
      ...
      <div className="text-right">
        ...
        <span className="ml-2 text-xs font-semibold px-2 py-0.5 bg-violet-100 text-violet-600 rounded-full">
          Unlimited
        </span>
      </div>
    </div>
...
export default function Dashboard() {
  ...
  // ── Derive real support usage from API response ─────────────────────────────
  const supportUsage = sub?.supportUsage;
  const isUnlimited = supportUsage?.allowedMinutes === "Unlimited";

  const usedMinutes =
    typeof supportUsage?.usedMinutes === "number"
      ? supportUsage.usedMinutes
      : 0;

  const allowedMinutes =
    !isUnlimited && typeof supportUsage?.allowedMinutes === "number"
      ? supportUsage.allowedMinutes
      : null;

  // Convert minutes → hours for display
  const usedHours = parseFloat((usedMinutes / 60).toFixed(1));
  const totalHours =
    allowedMinutes !== null
      ? parseFloat((allowedMinutes / 60).toFixed(1))
      : null;
  ...
  return (
    <>
      ...
      <div className="space-y-4 sm:space-y-5 px-3 sm:px-0">
        ...
        {/* Stat Cards */}
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4 border-gray-300">
          {[
            {
              label: "Current Plan",
            ...
            >
              ...
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${s.bg}`}
              >
                {s.icon}
              </div>
            </div>
            ...
            {[
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-md text-gray-900">{a.desc}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {subLoading ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-4 gap-2">
                  ...
                  <span
                    className={`px-3 py-1 text-md font-semibold rounded-full whitespace-nowrap flex-shrink-0 ${subscriptionBadgeClass}`}
                  >
                    {displayStatus}
                  </span>
                </div>
                ...
                <div className="space-y-3 mb-4">
                  {supportUsage ? (
                    isUnlimited ? (
                      // Enterprise: show badge instead of bar
                      <UnlimitedBadge usedMinutes={usedMinutes} />
                    ) : (
                      // Free / Pro: show progress bar
                      <UsageBar
                    ...
                    // No usage data yet — show empty bar at 0
                    <UsageBar
                      label="Support hours used"
                      used={0}
                      total={
          ...
          <div className="bg-white border border-gray-700 rounded-2xl p-4 sm:p-5">
            ...
            {invoicesLoading ? (
              ...
              <div className="text-center py-6 text-sm text-gray-900">
                No payments yet
              </div>
            ) : (
