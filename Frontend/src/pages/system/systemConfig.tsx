...
function ToggleRow({
  ...
  return (
    <div className="flex items-start justify-between gap-4 border border-gray-100 rounded-2xl px-5 py-4">
      <div>
        <p className="font-bold text-gray-900">{title}</p>
        <p className="text-md text-gray-700 mt-0.5">{description}</p>
      </div>
...
export default function SystemConfig() {
  ...
  const fetchConfig = async () => {
    try {
      ...
      setMaintenanceMode(Boolean(c.general?.maintenanceMode ?? false));
      setAutoBackup(Boolean(c.general?.autoBackup ?? true));
      ...
      setEmailNotifications(Boolean(c.email?.emailNotificationsEnabled ?? true));
      ...
      setRequire2FA(Boolean(c.security?.requireTwoFactorAuth ?? false));
      setAllowedIPs((c.security?.allowedIpAddresses ?? []).join("\n"));

      setNewUserRegistration(Boolean(c.notifications?.newUserRegistration ?? false));
      setServiceRequestAlerts(Boolean(c.notifications?.serviceRequestAlerts ?? false));
      setSystemErrors(Boolean(c.notifications?.systemErrors ?? false));
      setSecurityAlerts(Boolean(c.notifications?.securityAlerts ?? false));
      setSubscriptionRenewals(Boolean(c.notifications?.subscriptionRenewals ?? false));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {toast && (
        ...
        {/* Tab Switcher */}
        <div className="mb-6">
          <div className="flex bg-gray-200 rounded-full p-1 gap-1 overflow-x-auto scrollbar-none w-full sm:w-fit">
            {tabs.map((tab) => (
              <button
                key={tab.value}
                onClick={() => setActiveTab(tab.value)}
                className={`flex-shrink-0 px-3 sm:px-5 py-2 rounded-full text-xs sm:text-sm font-semibold transition-all whitespace-nowrap ${
                  activeTab === tab.value
