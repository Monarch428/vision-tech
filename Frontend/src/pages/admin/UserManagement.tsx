...
const inputCls = (err?: string) =>
  `w-full h-10 rounded-lg border px-3 text-sm outline-none transition-all ${err
    ? "border-red-400 bg-red-50 focus:border-red-500"
    : "border-gray-200 bg-gray-100 focus:border-green-500 focus:bg-white"
  }`;
...
function parseCSV(text: string): ImportedUser[] {
  ...
  const nameIdx = headers.findIndex((h) => ["name", "fullname", "full_name"].includes(h));
  const emailIdx = headers.findIndex((h) => h === "email");
  const passwordIdx = headers.findIndex((h) => ["password", "pass"].includes(h));
  ...
  return lines.slice(1).map((line) => {
    ...
    if (!user.email.trim() || !EMAIL_RE.test(user.email)) errs.push("invalid email");
    if (!user.password || user.password.length < 6) errs.push("password too short");
    if (!["admin", "user", "support"].includes(user.role)) errs.push(`invalid role "${user.role}"`);
    if (!["active", "inactive", "suspended"].includes(user.status)) errs.push(`invalid status "${user.status}"`);
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
      {status}
    </span>
  );
}
          ...
          <>
            ...
            >
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-4 py-2.5 text-sm transition-colors ${isActive ? "text-orange-500 hover:bg-orange-50" : "text-green-600 hover:bg-green-50"}`}
              >
                {isActive ? (
                  <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              ...
              <button
                onClick={() => { setOpen(false); onDeleteUser(); }}
...
function ImportPreviewTable({ users, onRemove }: { users: ImportedUser[]; onRemove: (idx: number) => void }) {
  return (
      ...
      <div className="overflow-x-auto max-h-56 overflow-y-auto">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-gray-50">
            {users.map((u, i) => (
              <tr key={i} className={u._rowError ? "bg-red-50" : "hover:bg-gray-50/60"}>
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`font-medium truncate max-w-[80px] ${u._rowError ? "text-red-700" : "text-gray-800"}`}>
                      {u.name || "—"}
                    </span>
                  </div>
                  {u._rowError && <p className="text-[10px] text-red-500 mt-0.5 leading-tight">{u._rowError}</p>}
                </td>
...
export default function UserManagement() {
  ...
  // Reset to page 0 when search/filter/itemsPerPage changes
  useEffect(() => {
    setCurrentPage(0);
  ...
  const handleFile = (file: File) => {
    ...
    if (!file.name.endsWith(".csv") && !file.name.endsWith(".txt")) {
      setImportError("Only CSV files are supported. Please download the template.");
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      if (!parsed.length) {
        setImportError("No valid rows found. Make sure the file has a header row and data rows.");
        return;
      }
  ...
  const handleBulkImport = async () => {
    ...
    if (!validUsers.length) {
      setImportError("No valid users to import. Fix errors or remove invalid rows.");
      return;
    }
    ...
    for (const u of validUsers) {
      try {
        await createUser({ name: u.name, email: u.email, password: u.password, role: u.role, status: u.status });
        succeeded++;
      } catch {
        failed++;
      }
    ...
    if (succeeded > 0) {
      setImportSuccess(`✓ ${succeeded} user${succeeded > 1 ? "s" : ""} imported successfully.${failed > 0 ? ` ${failed} failed.` : ""}`);
      setImportedUsers([]);
      fetchUsers();
    } else {
      setImportError(`All ${failed} imports failed. Check if emails already exist.`);
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      setEditLoading(true);
      setEditError("");
      const payload: Partial<FormState> = { name: editForm.name, email: editForm.email, role: editForm.role, status: editForm.status };
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-3 mb-6">
        <div className="min-w-0">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 leading-tight">User Management</h1>
          <p className="text-gray-700 mt-1 text-md">Manage your platform users, roles, and permissions.</p>
        </div>
        <button
          onClick={openAddModal}
          className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap shrink-0 mt-1"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3 p-4 border border-gray-700 border-b-0 bg-white rounded-t-xl">
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <svg className="absolute right-3 top-3.5 pointer-events-none" width="12" height="12" fill="none" stroke="#6b7280" strokeWidth="2.5" viewBox="0 0 24 24">
            <path d="m6 9 6 6 6-6" />
          </svg>
        </div>
      </div>
      ...
      {error && (
        <div className="px-4 py-3 bg-red-50 border-x border-red-200 text-sm text-red-600 flex items-center gap-2">
          {error}
      ...
      {/* ── Loading Skeleton ── */}
      {loading && (
        <div className="bg-white border border-x border-gray-100 p-6 space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex items-center gap-4 animate-pulse">
              ...
              <div className="h-6 bg-gray-100 rounded-full w-16 hidden sm:block" />
              <div className="h-6 bg-gray-100 rounded w-20 hidden sm:block" />
              <div className="h-6 bg-gray-100 rounded w-16 hidden sm:block" />
              <div className="h-6 bg-gray-100 rounded w-24 hidden sm:block" />
            </div>
          ))}
        </div>
      )}
      ...
      {!loading && (
        ...
        >
            ...
            {paginated.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-10">
                ...
                <p className="text-gray-400 text-xs">
                  {search || roleFilter ? "Try adjusting your search or filter" : "Add your first user to get started"}
                </p>
              </div>
            ) : (
              paginated.map((u) => (
                <div key={u._id} className="p-4 bg-white hover:bg-gray-50 transition-colors">
                  ...
                  <p className="mt-2 text-[11px] text-gray-400">
                    Last active: {u.lastLoginText || "Never"}
                  </p>
                </div>
              ))
            )}
          ...
          {/* Desktop Table View */}
          <TableContainer sx={{ maxHeight: 560 }} className="hidden sm:block">
            <Table stickyHeader aria-label="users table">
              <TableHead>
                <TableRow>
                  {["User", "Role", "Subscription", "Status", "Last Active", "Actions"].map((h) => (
                    <TableCell
                      key={h}
                      sx={{
                ...
                {paginated.length === 0 ? (
                  ...
                  paginated.map((u) => (
                    ...
                    >
                      ...
                      <TableCell sx={{ fontSize: "0.875rem", color: "#374151", fontWeight: 500 }}>
                        {u.subscription || "Free"}
                      </TableCell>
                      <TableCell><StatusBadge status={u.status} /></TableCell>
                      <TableCell sx={{ fontSize: "0.875rem", color: "#6b7280", whiteSpace: "nowrap" }}>
          ...
          <TablePagination
            ...
            page={currentPage}
            onPageChange={(_: unknown, newPage: number) => setCurrentPage(newPage)}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[90vh]">
            ...
            <div className="flex px-6 pb-3 gap-1 flex-shrink-0">
              <button
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${addTab === "manual" ? "bg-green-500 text-white" : "text-gray-500 hover:bg-gray-100"}`}
              >
                Add User
              </button>
              <button
                onClick={() => setAddTab("import")}
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 ${addTab === "import" ? "bg-green-500 text-white" : "text-gray-500 hover:bg-gray-100"}`}
              >
                <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            {addTab === "manual" && (
              <>
                <div className="overflow-y-auto flex-1 px-6">
                  <div className="space-y-3 pb-2">
                    <Field label="Full Name" error={addErrors.name}>
                      <input
                        ...
                        value={addForm.name}
                        onChange={(e) => { setAddForm((p) => ({ ...p, name: e.target.value })); if (addErrors.name) setAddErrors((p) => ({ ...p, name: undefined })); }}
                    ...
                    <Field label="Email" error={addErrors.email}>
                      <input
                        ...
                        value={addForm.email}
                        onChange={(e) => { setAddForm((p) => ({ ...p, email: e.target.value })); if (addErrors.email) setAddErrors((p) => ({ ...p, email: undefined })); }}
                    ...
                    <Field label="Password" error={addErrors.password}>
                      <input
                        ...
                        onChange={(e) => { setAddForm((p) => ({ ...p, password: e.target.value })); if (addErrors.password) setAddErrors((p) => ({ ...p, password: undefined })); }}
                    ...
                    <Field label="Role" error={addErrors.role}>
                      <select
                        value={addForm.role}
                        onChange={(e) => { setAddForm((p) => ({ ...p, role: e.target.value })); if (addErrors.role) setAddErrors((p) => ({ ...p, role: undefined })); }}
                  ...
                  {addError && <p className="mt-3 mb-2 text-xs text-red-500 bg-red-50 px-3 py-2 rounded-lg">{addError}</p>}
                </div>
                <div className="flex gap-2 px-6 py-4 flex-shrink-0 border-t border-gray-100">
                  <button onClick={() => setShowAdd(false)} className="flex-1 h-10 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                    Cancel
                  </button>
                  <button
              ...
              <>
                <div className="overflow-y-auto flex-1 px-6">
                  <div className="mb-3 flex items-center justify-between bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                    <div>
                      <p className="text-xs font-semibold text-blue-700">CSV Template</p>
                      <p className="text-[11px] text-blue-500 mt-0.5">name, email, password, role, status</p>
                    </div>
                    <button onClick={downloadTemplate} className="flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-800 transition-colors">
                  ...
                  <div className="mb-3 bg-gray-50 rounded-xl px-4 py-3 text-[11px] text-gray-500 space-y-1">
                    <p><span className="font-semibold text-gray-700">role:</span> admin · user · support</p>
                    <p><span className="font-semibold text-gray-700">status:</span> active · inactive · suspended</p>
                    <p><span className="font-semibold text-gray-700">password:</span> min. 6 characters</p>
                  </div>
                  {importedUsers.length === 0 && (
                    <div
                      ...
                      className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${importDragOver ? "border-green-400 bg-green-50" : "border-gray-200 hover:border-green-300 hover:bg-gray-50"}`}
                    >
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-700">{importDragOver ? "Drop to import" : "Drop your CSV here"}</p>
                  ...
                  {importedUsers.length > 0 && (
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => { setImportedUsers([]); setImportError(""); setImportSuccess(""); }}
                          className="ml-auto text-xs text-gray-400 hover:text-gray-600 underline transition-colors"
                        >
                          Clear & re-upload
                        </button>
                      </div>
                      <ImportPreviewTable users={importedUsers} onRemove={removeImportRow} />
                    </div>
                  )}
                  {importSuccess && (
                  ...
                  {importError && <p className="mt-3 text-xs text-red-500 bg-red-50 px-3 py-2 rounded-lg">{importError}</p>}
                </div>
                <div className="flex gap-2 px-6 py-4 flex-shrink-0 border-t border-gray-100">
                  <button onClick={() => setShowAdd(false)} className="flex-1 h-10 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                    Cancel
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center gap-2 px-6 pb-4 flex-shrink-0">
              {STEPS.map((label, idx) => (
                <div key={label} className="flex items-center gap-2 flex-1">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${idx < editStep ? "bg-green-500 text-white" : idx === editStep ? "bg-green-500 text-white ring-4 ring-green-100" : "bg-gray-100 text-gray-400"}`}>
                    {idx < editStep ? (
                  ...
                  <span className={`text-[11px] font-medium whitespace-nowrap ${idx === editStep ? "text-green-600" : "text-gray-400"}`}>{label}</span>
                  {idx < STEPS.length - 1 && <div className={`flex-1 h-px ${idx < editStep ? "bg-green-400" : "bg-gray-200"}`} />}
                </div>
              ))}
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6">
                ...
                <div className="pb-2">
                    ...
                    <div className="space-y-3">
                      <Field label="Full Name" error={editErrors.name}>
                        <input
                          type="text"
                          value={editForm.name}
                          onChange={(e) => { setEditForm((p) => ({ ...p, name: e.target.value })); if (editErrors.name) setEditErrors((p) => ({ ...p, name: undefined })); }}
                      ...
                      <Field label="Email" error={editErrors.email}>
                        <input
                          ...
                          onChange={(e) => { setEditForm((p) => ({ ...p, email: e.target.value })); if (editErrors.email) setEditErrors((p) => ({ ...p, email: undefined })); }}
                          className={inputCls(editErrors.email)}
                        />
                      </Field>
                      <Field label="New Password (leave blank to keep current)" error={editErrors.password}>
                        <input
                          type="password"
                          placeholder="Min. 6 characters"
                          value={editForm.password}
                          onChange={(e) => { setEditForm((p) => ({ ...p, password: e.target.value })); if (editErrors.password) setEditErrors((p) => ({ ...p, password: undefined })); }}
                  ...
                  {editStep === 1 && (
                    <div className="space-y-3">
                      <Field label="Role" error={editErrors.role}>
                        <select
                          value={editForm.role}
                          onChange={(e) => { setEditForm((p) => ({ ...p, role: e.target.value })); if (editErrors.role) setEditErrors((p) => ({ ...p, role: undefined })); }}
                      ...
                      <Field label="Status" error={editErrors.status}>
                        <select
                          ...
                          onChange={(e) => { setEditForm((p) => ({ ...p, status: e.target.value })); if (editErrors.status) setEditErrors((p) => ({ ...p, status: undefined })); }}
                  ...
                  {editStep === 2 && (
                    <div className="bg-gray-50 rounded-xl p-4 space-y-3 text-sm">
                      {[
                        { label: "Name", value: editForm.name, ok: !!editForm.name },
                        { label: "Email", value: editForm.email, ok: EMAIL_RE.test(editForm.email) },
                        { label: "Password", value: editForm.password ? "••••••••" : "Unchanged", ok: true },
                        { label: "Role", value: ROLES.find((r) => r.value === editForm.role)?.label || editForm.role, ok: !!editForm.role },
                        { label: "Status", value: STATUSES.find((s) => s.value === editForm.status)?.label || editForm.status, ok: !!editForm.status },
                      ].map((row) => (
              ...
              {editError && <p className="mt-3 mb-2 text-xs text-red-500 bg-red-50 px-3 py-2 rounded-lg">{editError}</p>}
            </div>

            <div className="flex gap-2 px-6 py-4 flex-shrink-0 border-t border-gray-100">
              {editStep > 0 && (
                <button onClick={prevStep} className="flex-1 h-10 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                  Back
                </button>
              )}
              {editStep < STEPS.length - 1 ? (
                <button onClick={nextStep} className="flex-1 h-10 bg-green-500 hover:bg-green-600 text-white rounded-xl text-sm font-semibold transition-colors">
                  Next
                </button>
              ) : (
                <button
                  onClick={handleUpdate}
                  disabled={editLoading || !editForm.name || !EMAIL_RE.test(editForm.email) || !editForm.role || !editForm.status}
