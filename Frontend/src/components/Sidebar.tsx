...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed top-0 left-0 h-screen w-[240px] bg-white border-r border-gray-300
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-3">
          {role === null ? (
            <p className="px-4 text-[12px] text-gray-400">Loading…</p>
          ) : (
            navGroups.map((group, gi) => (
              <div key={group.section} className="mb-1">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-[13.5px] font-semibold text-left transition-all
                        ${isActive
                ...
                {gi < navGroups.length - 1 && <div className="mx-4 my-2 h-px bg-gray-100" />}
              </div>
            ))
          )}
        </nav>
